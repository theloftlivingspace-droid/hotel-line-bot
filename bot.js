...
function filterByDate(row s, targetDate) {
...
const { checkIns, checkOuts } = filterByDate(r ows, tomorrow);
...
await saveUsers(users);
await lineReply(event.replyToke n, [{ type:  "text ", text:  `✅ ลงทะเบียนห้อง ${roomNum} สำเร็จแล้วค่ะ\n\nคุณจะได้รับการแจ้งเตือนค่าเช่าทาง LINE ทุกเดือนอัตโนมัติค่ะ`  }]);
...
: baseAmount + fine;
co nst totalStr = total.toLocaleString( "th-TH ", { minimumFractionDigits: 2 });
...
const total = totalAmount.toLo caleString( "th-TH ", { minimumFractionDigits: 2 });
...
function getBillingCycle(now=new Date()){
    ...
    // Start: วันสุดท้ายของเดือนก่อน
    const start = new Date(y, m, 0);
    
    // End: 8 วันก่อนวันที่ 7 ของเดือนถัดไป (buffer ก่อนครบกำหนด)
    const dueDateNextMonth = new Date(y, m + 1, 7); // วันที่ 7 เดือนหน้า
    const end = new Date(dueDateNextMonth);
    end.setDate(end.getDate() - 8); // ลบ 8 วัน
...
// หา confirm ed payment ในรอบนี้
...
const now = new Date();
const bangkokDate = new D ate(now.toLocaleString( "en-US ", { timeZone:  "Asia/Bangkok " }));
...
const endOfCycle=new Date(end); endOfCycle.setHours(2 3,59,59,999);
...
rooms[req.params.roomNumber] = { roomNumber: req.params.roomNumber, t enantName:  " ", amount: 0, invoiceLink:  " ", lineUserId:  " " };
}
...
amountMatch: true,
isSl ip: true, bankName: null, date: null,
...
await savePaym ents(payments.slice(0, 200));
...
app.post( "/api/upload-invoice ", adminAuth, upload.single( "file "), async (req, res) => {
  ...
  try {
    ...
    //  Clear/Archive payments เก่า (ก่อนรอบบิลปัจจุบัน)
    const { start } = getBillingCycle(now);
    const payments = await loadPayments();
    const paymentHistory = await redisGet("payment_history") || [];
    const oldPayments = payments.filter(p => new Date(p.receivedAt) < start);
    const newPayments = payments.filter(p => new Date(p.receivedAt) >= start);

    if (oldPayments.length > 0) {
      // Archive เก่าเก็บไว้ตรวจสอบย้อนหลัง (เก็บแค่ 12 เดือนล่าสุด)
      paymentHistory.unshift({ month: monthLabel, archivedAt: now.toISOString(), payments: oldPayments.map(p => ({...p, imageBase64: undefined})) }); // ลบรูป base64 ออกเพื่อประหยัดพื้นที่
      if (paymentHistory.length > 12) paymentHistory.pop();
      await redisSet("payment_history", paymentHistory);
      console.log(`[Cleanup] Archive payments เก่า ${oldPayments.length} รายการ ของเดือนก่อน`);
    }
    await savePayments(newPayments);
    console.log(`[Cleanup] Payments เหลือรอบปัจจุบัน: ${newPayments.length} รายการ`);
    // --- สิ้นสุดส่วน cleanup payments ---
    ...
    res.json({ ok: true, total: Object.keys(newRooms).length, archived: monthLabel, skipped: skipped.length, paymentsCleared: oldPayments.length, paymentsRemaining: newPayments.length });
...
await saveRooms(freshRooms); res.json({ ok: true, total: Object.keys(freshRooms).lengt h, archived: monthLabel });
});
...
for (const [userId, userRooms] o f Object.entries(grouped)) {
let messages;
if (userRooms.length === 1) {
const r = userRooms[0], amount = Number(r.amount).toLocaleString( "th-TH ", { minimumFractionDigits: 2 });
messages = [{ type:  "text ", text:  `คุณมีค่าเช่าห้อง ${r.roomNumber} เดือนนี้จำนวน ${amount} บาท ชำระภายในวันที่ 7 โอนผ่านบัญชีธนาคารไทยพาณิชย์ 353-2-05292-9 หรือ ธนาคารกสิกรไทย 799-2-39682-9 ชื่อบัญชี ณัฐวุฒิ จงจิตต` `าภิบาล ดูรายละเอียดกดลิงค์นี้ ${r.invoiceLink}`  }];
} else {
...
re s.json({ ok, fail, skipped, total: allRooms.length });
});
...
const amount = Number(room.amount).toLocaleString( "th-TH ", { minimumFractionDigits: 2 });
try { await linePush(room.lineUserId, [{ type:  "text ", text:  `คุณมีค่าเช่าห้อง ${room.roomNumber} เดือนนี้จำนวน ${amount} บาท ชำระภายในวันที่ 7 โอนผ่านบัญชีธนาคารไทยพาณิชย์ 353-2-05292-9 หรือ ธนาคารกสิกรไทย 799-2-39682-9 ชื่อบัญชี ณัฐวุฒิ จงจ` `ิตตาภิบาล ดูรายละเอียดกดลิงค์นี้ ${room.invoiceLink}`  }]); res.json({ ok: true }); }
